...
const (
	// BrocoindPre19 represents a brocoind version before 0.19.0.
	BrocoindPre19 BackendVersion = iota

	// BrocoindPost19 represents a brocoind version equal to or greater than
	// 0.19.0.
	BrocoindPost19

	// Brond represents a catch-all brond version.
	Brond
)
...
const (
	// brocoind19Str is the string representation of brocoind v0.19.0.
	brocoind19Str = "0.19.0"

	// brocoindVersionPrefix specifies the prefix included in every brocoind
	// version exposed through GetNetworkInfo.
	brocoindVersionPrefix = "/Bronees:"

	// brocoindVersionSuffix specifies the suffix included in every brocoind
	// version exposed through GetNetworkInfo.
	brocoindVersionSuffix = "/"
)

// parseBrocoindVersion parses the brocoind version from its string
// representation.
func parseBrocoindVersion(version string) BackendVersion {
	// Trim the version of its prefix and suffix to determine the
	// appropriate version number.
	version = strings.TrimPrefix(
		strings.TrimSuffix(version, brocoindVersionSuffix),
		brocoindVersionPrefix,
	)
	switch {
	case version < brocoind19Str:
		return BrocoindPre19
	default:
		return BrocoindPost19
	}
}
...
func (c *Client) BackendVersion() (BackendVersion, error) {
	...
	// to a brond backend if it does exist.
	info, err := c.GetInfo()

	switch err := err.(type) {
	// Parse the brond version and cache it.
	case nil:
	...
	case *bronjson.RPCError:
		if err.Code != bronjson.ErrRPCMethodNotFound.Code {
	...
	version := parseBrocoindVersion(networkInfo.SubVersion)
	c.backendVersion = &version
