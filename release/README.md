...
1. `git clone https://github.com/brsuite/brond.git
2. `cd brond`
...
6. Obtain a copy of `brond`'s source code with `git clone
   https://github.com/brsuite/brond` and checkout the source code of the
...
7. Proceed to verify the tag with `git verify-tag <TAG>` and compile the
   ...
   `BRONDBUILDSYS=OS-ARCH ./build/release/release.sh <TAG>`.
