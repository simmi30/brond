...
func Example_signMessage() {
	...
	privKey, pubKey := bronec.PrivKeyFromBytes(bronec.S256(), pkBytes)
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := bronec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := bronec.PrivKeyFromBytes(bronec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := bronec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
