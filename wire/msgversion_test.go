...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.BronDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.BronDecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.BronDecode with non *bytes.Buffer")
	}
	...
	for i, test := range tests {
		...
		err = msg.BronDecode(buf, test.pver, test.enc)
