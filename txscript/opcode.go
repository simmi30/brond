...
func opcodeCheckSig(op *parsedOpcode, vm *Engine) error {
	...
	pubKey, err := bronec.ParsePubKey(pkBytes, bronec.S256())
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
	}

	var signature *bronec.Signature
	if vm.hasFlag(ScriptVerifyStrictEncoding) ||
		vm.hasFlag(ScriptVerifyDERSignatures) {

		signature, err = bronec.ParseDERSignature(sigBytes, bronec.S256())
	} else {
		signature, err = bronec.ParseSignature(sigBytes, bronec.S256())
	}
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
...
type parsedSigInfo struct {
	signature       []byte
	parsedSignature *bronec.Signature
	parsed          bool
}
...
func opcodeCheckMultiSig(op *parsedOpcode, vm *Engine) error {
	...
	for numSignatures > 0 {
		...
		var parsedSig *bronec.Signature
		if !sigInfo.parsed {
			if err := vm.checkHashTypeEncoding(hashType); err != nil {
				return err
			}
			...
			if vm.hasFlag(ScriptVerifyStrictEncoding) ||
				...
				parsedSig, err = bronec.ParseDERSignature(signature,
					bronec.S256())
			} else {
				parsedSig, err = bronec.ParseSignature(signature,
					bronec.S256())
			}
			sigInfo.parsed = true
			if err != nil {
		...
		// Parse the pubkey.
		parsedPubKey, err := bronec.ParsePubKey(pubKey, bronec.S256())
		if err != nil {
			continue
		}
