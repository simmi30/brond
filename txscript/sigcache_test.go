...
// test data.
func genRandomSig() (*chainhash.Hash, *bronec.Signature, *bronec.PublicKey, error) {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	sig1Copy, _ := bronec.ParseSignature(sig1.Serialize(), bronec.S256())
	key1Copy, _ := bronec.ParsePubKey(key1.SerializeCompressed(), bronec.S256())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		sigCopy, _ := bronec.ParseSignature(sig.Serialize(), bronec.S256())
		keyCopy, _ := bronec.ParsePubKey(key.SerializeCompressed(), bronec.S256())
	...
	sigNewCopy, _ := bronec.ParseSignature(sigNew.Serialize(), bronec.S256())
	keyNewCopy, _ := bronec.ParsePubKey(keyNew.SerializeCompressed(), bronec.S256())
