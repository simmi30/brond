...
func commandUsage(method string) {
	usage, err := bronjson.MethodUsageText(method)
	if err != nil {
...
func main() {
	...
	method := args[0]
	usageFlags, err := bronjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := bronjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// bronjson.Error as it reallistcally will always be since the
		...
		// The error is not a bronjson.Error and this really should not
	...
	marshalledJSON, err := bronjson.MarshalCmd(1, cmd)
	if err != nil {
