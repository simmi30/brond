...
func TestMsgAlert(t *testing.T) {
	...
	// TestBronEncode with Payload == nil
	var buf bytes.Buffer
	err := msg.BronEncode(&buf, pver, encoding)
	if err != nil {
		t.Error(err.Error())
	}
	...
	if !bytes.Equal(buf.Bytes(), expectedBuf) {
		t.Errorf("BronEncode got: %s want: %s",
...
func TestMsgAlertWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		var buf bytes.Buffer
		err := test.in.BronEncode(&buf, test.pver, test.enc)
		if err != nil {
			t.Errorf("BronEncode #%d error %v", i, err)
			continue
		}
		if !bytes.Equal(buf.Bytes(), test.buf) {
			t.Errorf("BronEncode #%d\n got: %s want: %s", i,
		...
		err = msg.BronDecode(rbuf, test.pver, test.enc)
		if err != nil {
			t.Errorf("BronDecode #%d error %v", i, err)
			continue
		}
		if !reflect.DeepEqual(&msg, test.out) {
			t.Errorf("BronDecode #%d\n got: %s want: %s", i,
...
func TestMsgAlertWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := test.in.BronEncode(w, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.writeErr) {
			t.Errorf("BronEncode #%d wrong error got: %v, want: %v",
				i, err, test.writeErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("BronEncode #%d wrong error got: %v, "+
		...
		err = msg.BronDecode(r, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.readErr) {
			t.Errorf("BronDecode #%d wrong error got: %v, want: %v",
				i, err, test.readErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.readErr {
				t.Errorf("BronDecode #%d wrong error got: %v, "+
	...
	w := new(bytes.Buffer)
	err := baseMsgAlert.BronEncode(w, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.BronEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
	...
	buf := *new(bytes.Buffer)
	err = baseMsgAlert.BronEncode(&buf, pver, encoding)
	if _, ok := err.(*MessageError); !ok {
		t.Errorf("MsgAlert.BronEncode wrong error got: %T, want: %T",
			err, MessageError{})
	}
