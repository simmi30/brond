...
type notificationState struct {
	...
	notifySpent        map[bronjson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[bronjson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[bronjson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *bronutil.Tx, details *bronjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *bronutil.Tx, details *bronjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAcceptedVerbose func(txDetails *bronjson.TxRawResult)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case bronjson.BlockConnectedNtfnMethod:
	...
	case bronjson.FilteredBlockConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBlockDisconnected
	case bronjson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case bronjson.FilteredBlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRecvTx
	case bronjson.RecvTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRedeemingTx
	case bronjson.RedeemingTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case bronjson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanFinished
	case bronjson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case bronjson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case bronjson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case bronjson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBrondConnected
	case bronjson.BrondConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		if c.ntfnHandlers.OnBrondConnected == nil {
			return
		}

		connected, err := parseBrondConnectedNtfnParams(ntfn.Params)
		if err != nil {
		...
		c.ntfnHandlers.OnBrondConnected(connected)

	// OnAccountBalance
	case bronjson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case bronjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseChainTxNtfnParams(params []json.RawMessage) (*bronutil.Tx,
	*bronjson.BlockDetails, error) {
	...
	// JSON object.
	var block *bronjson.BlockDetails
	if len(params) > 1 {
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*bronjson.TxRawResult,
	error) {
	...
	var rawTx bronjson.TxRawResult
...
// parseBrondConnectedNtfnParams parses out the connection status of brond
// and bronwallet from the parameters of a brondconnected notification.
func parseBrondConnectedNtfnParams(params []json.RawMessage) (bool, error) {
	if len(params) != 1 {
...
func (c *Client) notifySpentInternal(outpoints []bronjson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := bronjson.NewNotifySpentCmd(outpoints)
	return c.sendCmd(cmd)
}

// newOutPointFromWire constructs the bronjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) bronjson.OutPoint {
	return bronjson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]bronjson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := bronjson.NewNotifySpentCmd(ops)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := bronjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := bronjson.NewNotifyReceivedCmd(addresses)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyReceivedAsync(addresses []bronutil.Address) FutureNotifyReceivedResult {
	...
	cmd := bronjson.NewNotifyReceivedCmd(addrs)
	return c.sendCmd(cmd)
}
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := bronjson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	...
	cmd := bronjson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.sendCmd(cmd)
}
...
func (c *Client) LoadTxFilterAsync(reload bool, addresses []bronutil.Address,
	...
	outPointObjects := make([]bronjson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = bronjson.OutPoint{
			Hash:  outPoints[i].Hash.String(),
			Index: outPoints[i].Index,
		}
	}

	cmd := bronjson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
