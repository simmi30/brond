...
type addressToKey struct {
	key        *bronec.PrivateKey
	compressed bool
}

func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr bronutil.Address) (*bronec.PrivateKey,
			bool, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*bronec.PublicKey)(&key.PublicKey).
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key1, err := bronec.NewPrivateKey(bronec.S256())
			if err != nil {
				t.Errorf("failed to make privKey for %s: %v",
					msg, err)
				break
			...
			pk1 := (*bronec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			...
			key2, err := bronec.NewPrivateKey(bronec.S256())
			if err != nil {
			...
			pk2 := (*bronec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
...
func TestSignatureScript(t *testing.T) {
	...
	privKey, _ := bronec.PrivKeyFromBytes(bronec.S256(), privKeyD)
