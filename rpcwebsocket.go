...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := bronjson.NewBlockConnectedNtfn(block.Hash().String(), block.Height(),
		block.MsgBlock().Header.Timestamp.Unix())
	marshalledJSON, err := bronjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *bronutil.Block) {
	...
	ntfn := bronjson.NewBlockDisconnectedNtfn(block.Hash().String(),
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := bronjson.NewFilteredBlockConnectedNtfn(block.Height(),
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := bronjson.NewFilteredBlockDisconnectedNtfn(block.Height(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *bronutil.Tx) {
	...
	ntfn := bronjson.NewTxAcceptedNtfn(txHashStr, bronutil.Amount(amount).ToBRON())
	marshalledJSON, err := bronjson.MarshalCmd(nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *bronjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = bronjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = bronjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
...
// blockDetails creates a BlockDetails struct to include inbronws notifications
// from a block and a transaction's block index.
func blockDetails(block *bronutil.Block, txIndex int) *bronjson.BlockDetails {
	if block == nil {
		return nil
	}
	return &bronjson.BlockDetails{
		Height: block.Height(),
...
func newRedeemingTxNotification(txHex string, index int, block *bronutil.Block) ([]byte, error) {
	...
	ntfn := bronjson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return bronjson.MarshalCmd(nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			ntfn := bronjson.NewRecvTxNtfn(txHex, blockDetails(block,
				tx.Index()))
...
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *bronutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		n := bronjson.NewRelevantTxAcceptedNtfn(txHexString(tx.MsgTx()))
		marshalled, err := bronjson.MarshalCmd(nil, n)
		if err != nil {
...
out:
	for {
		...
		switch authCmd, ok := cmd.cmd.(*bronjson.AuthenticateCmd); {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bronjson.HelpCmd)
	if !ok {
		return nil, bronjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*bronjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &bronjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bronjson.NotifySpentCmd)
	if !ok {
		return nil, bronjson.ErrRPCInternal
	}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bronjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, bronjson.ErrRPCInternal
	}
...
func handleNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bronjson.NotifyReceivedCmd)
	if !ok {
		return nil, bronjson.ErrRPCInternal
	}
...
func handleStopNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bronjson.StopNotifySpentCmd)
	if !ok {
		return nil, bronjson.ErrRPCInternal
	}
...
func handleStopNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bronjson.StopNotifyReceivedCmd)
	if !ok {
		return nil, bronjson.ErrRPCInternal
	}
...
func deserializeOutpoints(serializedOuts []bronjson.OutPoint) ([]*wire.OutPoint, error) {
...
var ErrRescanReorg = bronjson.RPCError{
	Code:    bronjson.ErrRPCDatabase,
	Message: "Reorganize",
}
	...
	for _, tx := range blk.Transactions() {
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				ntfn := bronjson.NewRecvTxNtfn(txHex,
...
func handleRescanBlocks(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bronjson.RescanBlocksCmd)
	if !ok {
		return nil, bronjson.ErrRPCInternal
	}
	...
	discoveredData := make([]bronjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, bronjson.RescannedBlock{
...
func scanBlockChunks(wsc *wsClient, cmd *bronjson.RescanCmd, lookups *rescanKeys, minBlock,
...
fetchRange:
	for minBlock < maxBlock {
		...
		if err != nil {
			rpcsLog.Errorf("Error looking up block range: %v", err)
			return nil, nil, &bronjson.RPCError{
				Code:    bronjson.ErrRPCDatabase,
				Message: "Database error: " + err.Error(),
			}
		}
	...
	loopHashList:
		for i := range hashList {
			...
			n := bronjson.NewRescanProgressNtfn(
				hashList[i].String(), blk.Height(),
				blk.MsgBlock().Header.Timestamp.Unix(),
			)
			mn, err := bronjson.MarshalCmd(nil, n)
			if err != nil {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*bronjson.RescanCmd)
	if !ok {
		return nil, bronjson.ErrRPCInternal
	}
